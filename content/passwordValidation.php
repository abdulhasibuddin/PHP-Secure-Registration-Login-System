<?php
    ...
    if($password != ""){ //If password field is not empty [for first time load of the page]
        ...
        if (!preg_match_all('$\S*(?=\S{8,})\S*$', $password)) { //Check if the password contains atleast 8 characters
            $pass_size_min = "<div style='color: red;'>*Password should be atleast 8 characters!</div>";
            $errFlag = 1;
        }
        if (!preg_match_all('$\S*(?=\S{1,255})\S*$', $password)) { //Check if the password contains more than 255 characters
            $pass_size_max = "<div style='color: red;'>*Password should not exceed 255 characters!</div>";
            $errFlag = 1;
        }
